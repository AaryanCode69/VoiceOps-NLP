 ...
 */
export class VoiceOpsAnalyzeCall implements INodeType {
		...
		properties: [
			{
				...
				description:
					'Risk score threshold (0–100). Scores >= threshold route to High Risk, scores < threshold route to Normal.',
			},
		],
	};
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				const fraudLikelihood: string = riskAssessment.fraud_likelihood ?? 'unknown';
